...
parser.add_argument('-w', '--writer', action='store_true', help='is writer serial (or reciever) ')
...
def recieveData(ser):
    ...
    ret = ''
    dataWaiting = ser.in_waiting
    if dataWaiting > 0:
        ret = ser.read(dataWaiting)
    return ret
...
if __name__=='__main__':
    ...
    if isWriter:
        print('serial writer initialized...')
        while True:
            time.sleep(0.0001)
            #dataSum += sendData(ser, oneSecData)

            dataSum += sendData(ser, b'00112233445566'*20)
            if time.time() - tic >= 5:
                bps = (dataSum*8)/(time.time() - tic)
                print('%s sent bps: %0.2f[bps] effective baud rate: %0.2f'%(port, bps, bps/baud) ) 
                tic = time.time()
                dataSum = 0.0
    else:
        print('serial reader initialized...')
        while True:
            time.sleep(0.0001)
            ...
            if time.time() - tic >= 5:
                bps = (dataSum*8)/(time.time() - tic)
                print('%s recieved bps: %0.2f[bps] effective baud rate: %0.2f'%(port, bps, bps/baud) ) 
                tic = time.time()
                dataSum = 0.0
